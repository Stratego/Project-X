...
public class GeneradorImagenJugador {
    ...
    public static ArrayList<ElementoDibujable> generarTexturas(Color color,DireccionJugador direccion){
       ArrayList<ElementoDibujable> texturas  = new ArrayList<ElementoDibujable>();
        texturas.add(generarRopa(color,direccion));
        texturas.add(generarAspecto(direccion));
        texturas.add(generarPerfil(direccion));

        return texturas;
    }

    public static ArrayList<ElementoDibujable> generarTexturasIntefaz(Color color,DireccionJugador direccion){
        ArrayList<ElementoDibujable> texturas  = new ArrayList<ElementoDibujable>();
        texturas.add(generarRopaIntefaz(color,direccion));
        texturas.add(generarAspectoInterfaz(direccion));
    ...
    private static ElementoDibujable generarAspectoInterfaz(DireccionJugador direccion){
    ...
    private static ElementoDibujable generarAspecto(DireccionJugador direccion){
        Random random = new Random();
        int tipoPiel = random.nextInt(2)+1;
        String textura = "jugador/piel/jugador"+generarDireccion(direccion)+"pell"+tipoPiel+".png";
        return new ElementoDibujable(TipoDibujo.elementosJuego,textura);
